"""Integration tests for AgentRuntime with mocked LLM transport."""
...
@pytest.mark.asyncio
async def test_simple_end_turn() -> None:
    settings = _make_settings()
    runtime = AgentRuntime(settings=settings)
    try:
        with patch.object(
            runtime, "_call_with_retry", new=AsyncMock(return_value=_make_text_response("Hola!"))
        ):
...
@pytest.mark.asyncio
async def test_tool_use_tier1_auto() -> None:
    ...
    try:
        ...
        with patch.object(
            runtime, "_call_with_retry", new=AsyncMock(side_effect=[tool_resp, text_resp])
        ):
            text, trace = await runtime.run(
...
@pytest.mark.asyncio
async def test_tool_use_tier3_blocked() -> None:
    from emergent.tools.registry import SafetyTier, ToolDefinition, ToolRegistry
...
@pytest.mark.asyncio
async def test_history_is_passed_to_llm() -> None:
    settings = _make_settings()
    runtime = AgentRuntime(settings=settings)
    try:
        history = [
